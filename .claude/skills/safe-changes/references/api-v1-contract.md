...
⚠️ `host_profile = event.host_user.host_profile` is unguarded — nil `host_user` (board_submission/scouted provenance) raises. Any fix must keep the `host` key present (nullable sub-fields), not remove it.
...
`{ event: <event object> }`; 404 `{ error }`. ⚠️ No `publicly_visible` check on the event lookup itself.
...
```
⚠️ Filters only `e.active?` — missing `publicly_visible` gate (known gap).
...
```
⚠️ Filters `status: :active` only — missing `publicly_visible` gate (known gap).
